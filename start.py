    ...
    def lineReceived(self, line):
        ...
        if data.get('id') == 1:
            ...
            if data.get('result', {}).get('job'):
                self.factory.di.to_client.put(toJson({
                    'type': 'job',
...
class ProxyServer(autobahn.twisted.websocket.WebSocketServerProtocol):
    ...
    def onMessage(self, data, isBinary):
        ...
        if data.get('type') == 'auth':
            ...
            self.di.to_server.put(toJson({
                'method': 'login',
                'params': {
                    'login': login,
                    'pass': self.authPass
                },
                'id': self.di.getNextRpcId()
            }))
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        'pool_port',
        metavar='stratum_port',
        help='stratum tcp port',
        type=int,
    ...
    parser.add_argument(
        '--ssl',
        help='Enables SSL for WebSocket clients (privatekey:certificate)',
        metavar='CERTIFICATES',
        dest='ssl'
    )
    parser.add_argument('--pool_pass', help='Stratum auth password (default: x)', default='x')
    parser.add_argument(
        '--password',
        help='Password for the stats page (default: no password)',
        default=None
    )
    ...
    if arguments['ssl']:
        ...
        try:
            contextFactory = ssl.DefaultOpenSSLContextFactory(
                arguments['ssl'].split(':')[0],
                arguments['ssl'].split(':')[1]
            )
